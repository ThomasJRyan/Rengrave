...
- When adding or modifying documentation that explains how to use a tool or UI workflow, include a current marked screenshot that demonstrates the relevant steps. For egui/eframe screens, capture the rendered surface with the repository's egui screenshot functionality (the inspection `Screenshot` endpoint or an egui harness snapshot) after constructing a representative populated state; semantic-tree results alone are not visual evidence. Use the `imagegen` skill's built-in image-editing workflow on that screenshot to add clear circles, arrows, lines, numbered callouts, text, or other annotations needed to make the steps unambiguous. Preserve the underlying UI, labels, proportions, and legibility, and do not invent controls or states that were not captured. Keep the unmarked source and final marked image under `docs/` when both are useful, reference the marked image from the RST with descriptive alt text/caption, and verify that it appears in the generated documentation. If the UI cannot be rendered in the current environment, record the exact limitation and do not claim or fabricate screenshot evidence; use an existing screenshot only when it is verified to represent the current UI.
