...
use rengrave_core::design::{DesignCircle, DesignGeometry, DesignObjectId, VectorDocument};
...
impl GeneralDesignTool {
    fn accessible_label(self) -> &'static str {
        match self {
            ...
            Self::EditParameters => "Edit Vector Parameters",
        }
    }
    ...
    fn icon_strokes(self) -> &'static [Stroke] {
        match self {
            ...
            Self::EditParameters => general_edit_parameters_icon_strokes(),
        }
    }
}
...
impl CircleDraft {
    fn from_circle(circle: DesignCircle) -> Self {
        Self {
...
struct RengraveApp {
    ...
    general_selected_toolpath: Option<u64>,
    next_general_toolpath_id: u64,
    ...
    general_toolbit_library: GeneralToolbitLibrary,
    general_toolbit_library_path: Option<PathBuf>,
    show_general_toolbit_library: bool,
    general_toolbit_selected_id: Option<String>,
...
impl RengraveApp {
    fn new(cc: &eframe::CreationContext<'_>, options: UiLaunchOptions) -> Self {
        ...
        let mut app = Self {
            ...
            general_selected_toolpath: None,
            next_general_toolpath_id: 1,
            general_2d_transform: ViewTransform {
            ...
            general_toolbit_library: default_general_toolbit_library_path()
                .as_deref()
                .and_then(|path| GeneralToolbitLibrary::load(path).ok())
                .unwrap_or_default(),
            general_toolbit_library_path: default_general_toolbit_library_path(),
            show_general_toolbit_library: false,
            general_toolbit_selected_id: None,
    ...
    fn apply_project_file(
        ...
        self.general_selected_toolpath = None;
...
impl RengraveApp {
    ...
    fn general_scene(&self) -> GeneralScene {
        ...
        let edit_target = self.general_temporary_edit_target();
        for object in self.general_design_document.objects() {
            if edit_target == Some(object.id) {
                continue;
            }
    ...
    fn general_temporary_edit_target(&self) -> Option<DesignObjectId> {
        match self.general_temporary_tool {
    ...
    fn begin_general_object_edit(&mut self, id: DesignObjectId) {
        let Some(object) = self.general_design_document.object(id) else {
            return;
        };
        let DesignGeometry::Circle(circle) = object.geometry;
    ...
    fn show_general_toolpath_panel(&mut self, ui: &mut egui::Ui) {
                    ...
                    if let Some(mut tool) = tool {
                        ...
                        match source.map(|source| {
                            ...
                            Some(Ok(GeneratedProfile { contours, gcode })) => {
                                let toolpath_id =
                                    session.editing_toolpath_id.unwrap_or_else(|| {
                                ...
                                let record = GeneralToolpathRecord {
                                    ...
                                    enabled: self
                                        .general_toolpaths
                                        .iter()
                                        .find(|record| record.id == toolpath_id)
                                ...
                                self.general_selected_toolpath = Some(toolpath_id);
        ...
        let top_height = ((ui.available_height() - 8.0) * 0.5).max(150.0);
        ...
        ui.separator();
        self.show_general_toolpath_list(ui);
    }
    ...
    fn show_general_2d_view(&mut self, ui: &mut egui::Ui, rect: egui::Rect) {
            ...
            && self.general_design_document.remove_object(id)
        {
            ...
            if self.general_selected_toolpath.is_some_and(|toolpath_id| {
                !self
                    .general_toolpaths
                    .iter()
                    .any(|record| record.id == toolpath_id)
            }) {
                self.general_selected_toolpath = None;
            }
            if self
        ...
        let canvas_label = if self.general_toolpaths.iter().all(|record| !record.enabled) {
            "2D design view"
        } else {
    ...
    fn show_general_design_objects(
        ...
        for object in self.general_design_document.objects() {
                    ...
                    .is_some_and(|pointer| pointer.distance(center) <= interaction_radius)
            {
                ...
                edit_requested |= response.double_clicked();
            }
        ...
        if let Some(GeneralTemporaryTool::Circle(session)) = self.general_temporary_tool {
            ...
            let radius =
                (general_display_value(session.draft.radius_mm, self.general_job_setup.units)
                    * self.general_2d_transform.zoom) as f32;
        ...
        if let Some(pointer) = selection_pointer {
            ...
            self.general_selected_object = selected;
            if edit_requested && let Some(id) = selected {
                self.begin_general_object_edit(id);
            }
    ...
    fn show_general_3d_view(&mut self, ui: &mut egui::Ui, rect: egui::Rect) {
        ...
        let viewport_label = if self.general_toolpaths.iter().all(|record| !record.enabled) {
            "3D job view"
        } else {
    ...
    fn show_general_design_tools(&mut self, ui: &mut egui::Ui) {
        if let Some(mut temporary_tool) = self.general_temporary_tool.take() {
            let action = match &mut temporary_tool {
                GeneralTemporaryTool::Circle(session) => {
                    show_general_circle_settings(ui, session, self.general_job_setup.units)
                }
            };
            match action {
                Some(TemporaryToolAction::Confirm) => {
                    let GeneralTemporaryTool::Circle(session) = temporary_tool;
                    match session.mode {
                        CircleToolMode::Create => {
                            if let Ok(id) = self
                                .general_design_document
                                .add_circle(session.draft.center_mm, session.draft.radius_mm)
                            {
                                self.general_selected_object = Some(id);
                            }
                        }
                        CircleToolMode::Edit(id) => {
                            if self
                                .general_design_document
                                .update_circle(id, session.draft.center_mm, session.draft.radius_mm)
                                .is_ok()
                            {
                                self.general_selected_object = Some(id);
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                general_setup_group(ui, "Transform Objects", |ui, content_width| {
                    let _ = general_design_tool_grid(
                        ui,
                        "transform_objects",
                        &[],
                        content_width,
                        self.general_selected_object.is_some(),
                    );
                });
                general_setup_group(ui, "Edit Objects", |ui, content_width| {
                    if let Some(tool) = general_design_tool_grid(
                        ui,
                        "edit_objects",
                        &[GeneralDesignTool::EditParameters],
                        content_width,
                        self.general_selected_object.is_some(),
                    ) {
        ...
        match selected_tool {
            ...
            Some(GeneralDesignTool::EditParameters) => {
                if let Some(id) = self.general_selected_object {
                    self.begin_general_object_edit(id);
                }
            }
            None => {}
    ...
    fn show_general_toolbit_library_window(&mut self, ctx: &egui::Context) {
        if !self.show_general_toolbit_library {
            return;
        }
        if self.general_toolbit_selected_id.is_none() {
            self.general_toolbit_selected_id = self
                .general_toolbit_library
                .toolbits
                .first()
                .map(|tool| tool.id.clone());
    ...
    fn selected_general_toolbit_index(&self) -> Option<usize> {
        let id = self.general_toolbit_selected_id.as_deref()?;
        self.general_toolbit_library
            .toolbits
            .iter()
    ...
    fn next_general_toolbit_id(&self) -> String {
        let mut number = self.general_toolbit_library.toolbits.len() + 1;
        loop {
            let id = format!("general-toolbit-{number}");
            if self
                .general_toolbit_library
                .toolbits
                .iter()
    ...
    fn show_general_toolbit_delete_confirmation(&mut self, ctx: &egui::Context) {
        let Some(id) = self.general_toolbit_delete_id.clone() else {
            return;
        };
        let label = self
            .general_toolbit_library
            ...
            .unwrap_or_else(|| "this toolbit".to_owned());
        let mut open = true;
        egui::Window::new("Delete toolbit?")
            .open(&mut open)
            .collapsible(false)
            .resizable(false)
            .show(ctx, |ui| {
                ui.label(format!(
                    "Delete `{label}` from the General toolbit library?"
                ));
                ui.horizontal(|ui| {
                    if ui.button("Delete").clicked() {
                        ...
                        if self.general_toolbit_selected_id.as_deref() == Some(id.as_str()) {
                            self.general_toolbit_selected_id = self
                                .general_toolbit_library
                                .toolbits
                                .first()
                        ...
                        self.general_toolbit_delete_id = None;
                        self.save_general_toolbit_library();
                    }
    ...
    fn import_general_toolbit_library(&mut self) -> bool {
        let Some(path) = FileDialog::new()
            .add_filter("R-Engrave JSON", &["json"])
            .pick_file()
        else {
            return false;
        };
        match GeneralToolbitLibrary::load(&path) {
            Ok(library) => {
                let mut imported = 0;
                for mut tool in library.toolbits {
                    if self
                        .general_toolbit_library
                        .toolbits
                        .iter()
                ...
                self.status = format!("Imported {imported} toolbit(s)");
                imported > 0
            }
            Err(error) => {
                self.status = error;
    ...
    fn export_general_toolbit_library(&mut self) {
        let Some(path) = FileDialog::new()
            .add_filter("R-Engrave JSON", &["json"])
            .set_file_name("general_toolbits.json")
            .save_file()
        else {
            return;
        };
        self.status = match self.general_toolbit_library.save_atomic(&path) {
            Ok(()) => format!("Exported toolbit library to {}", path.display()),
            Err(error) => error,
        };
    }
            ...
            if self.screen == AppScreen::Workbench {
                ui.menu_button("Settings", |ui| {
                    if menu_action(
                        ui,
                        "Toolbit Library…",
                        self.tool_view == ToolView::GeneralPurpose,
                    ) {
                        self.show_general_toolbit_library = true;
                    }
                });
            }
...
fn general_edit_parameters_icon_strokes() -> &'static [Stroke] {
    static STROKES: OnceLock<Vec<Stroke>> = OnceLock::new();
    STROKES
        .get_or_init(|| {
            parse_svg_segments(include_str!("../assets/tool-icons/edit-parameters.svg"))
                .expect("the bundled edit-parameters tool icon must be valid SVG")
        })
        .as_slice()
}
                ...
                ui.horizontal(|ui| {
                    ...
                    for column in 0..GENERAL_DESIGN_GRID_COLUMNS {
                        ...
                        if let Some(tool) = tools.get(index).copied() {
                            let response = ui
                                .add_enabled_ui(enabled, |ui| {
                                    general_design_tool_button(ui, tool, button_size)
                                })
                                .inner;
                            if response.clicked() {
                                selected_tool = Some(tool);
...
fn show_general_circle_settings(
    ui: &mut egui::Ui,
    session: &mut CircleToolSession,
    units: GeneralUnits,
) -> Option<TemporaryToolAction> {
    let (title, confirm_label) = match session.mode {
        CircleToolMode::Create => ("Create Circle", "Create"),
        CircleToolMode::Edit(_) => ("Edit Circle", "Update"),
    };
...
fn show_general_temporary_settings(
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            add_contents(ui);
            ui.add_space(2.0);
            action = general_temporary_action_row(ui, confirm_label).or(action);
        });
    action
}

fn general_temporary_action_row(
    ui: &mut egui::Ui,
    confirm_label: &str,
) -> Option<TemporaryToolAction> {
...
fn general_toolbit_optional_length_row(
    ui: &mut egui::Ui,
    label: &str,
...
fn general_toolbit_optional_angle_row(
    ui: &mut egui::Ui,
    label: &str,
...
mod tests {
    ...
    fn test_app() -> RengraveApp {
        ...
        RengraveApp {
            ...
            show_recent_projects: false,
            show_new_project_modal: false,
            general_toolbit_library: GeneralToolbitLibrary::default(),
            general_toolbit_library_path: None,
            show_general_toolbit_library: false,
            general_toolbit_selected_id: None,
            general_toolbit_filter: String::new(),
            general_toolbit_units: GeneralUnits::Millimetres,
    ...
    fn kittest_profile_generation_populates_gcode_and_both_viewports() {
        ...
        harness
            ...
            .click();
        harness.run();
        assert!(!harness.state().general_toolpaths[0].enabled);
        let state = harness.state();
        assert!(
            harness
    ...
    fn kittest_circle_tool_previews_commits_cancels_and_selects() {
        ...
        if let Some(GeneralTemporaryTool::Circle(session)) =
            &mut harness.state_mut().general_temporary_tool
        {
            session.draft.center_mm = Point::new(12.0, -8.0);
            session.draft.radius_mm = 6.0;
        }
    ...
    fn kittest_edit_parameters_requires_selection_and_updates_without_saving() {
        ...
        let project_path = std::env::temp_dir().join(format!(
            "rengrave-edit-does-not-autosave-{}.rgrv",
            std::process::id()
        ));
        ...
        harness.get_by_label("Edit Vector Parameters").click();
        harness.run();
        assert!(harness.state().general_temporary_tool.is_none());
        ...
        let id = harness
            .state_mut()
            .general_design_document
            .add_circle(Point::new(4.0, -2.0), 5.0)
            .expect("valid editable circle");
        harness.state_mut().general_selected_object = Some(id);
        harness.run();
        harness.get_by_label("Edit Vector Parameters").click();
        harness.run();

        assert!(harness.query_by_label("Edit Circle").is_some());
        assert!(harness.query_by_label("Update").is_some());
        ...
        assert!(matches!(
            ...
            Some(GeneralTemporaryTool::Circle(CircleToolSession {
                ...
                mode: CircleToolMode::Edit(target),
            })) if target == id
        ));
        assert!(matches!(
            harness.state().general_scene().objects.as_slice(),
        ...
        if let Some(GeneralTemporaryTool::Circle(session)) =
            &mut harness.state_mut().general_temporary_tool
        {
            session.draft.center_mm = Point::new(20.0, 10.0);
            session.draft.radius_mm = 9.0;
        }
        harness.get_by_label("Cancel").click();
        harness.run();
        assert_eq!(
            harness
                ...
                .general_design_document
                .object(id)
                .expect("unchanged circle")
                .geometry,
            DesignGeometry::Circle(DesignCircle {
                center_mm: Point::new(4.0, -2.0),
                radius_mm: 5.0,
            })
        );
        ...
        let updated = harness
            ...
            .general_design_document
            .object(id)
            .expect("updated circle");
        assert_eq!(updated.id, id);
        assert_eq!(
            updated.geometry,
            DesignGeometry::Circle(DesignCircle {
                center_mm: Point::new(20.0, 10.0),
                radius_mm: 9.0,
            })
        );
        assert!(
            !project_path.exists(),
            "editing must not save automatically"
        );
        let (project, _) = harness
            .state()
            .current_project_file()
            .expect("saveable project snapshot");
        assert_eq!(
            project
                .design_document
                .object(id)
                .expect("persisted updated circle")
                .geometry,
            updated.geometry
        );
    }
    ...
    #[test]
    fn kittest_vector_edit_session_opens_with_selected_object_parameters() {
        let mut app = test_app();
        app.tool_view = ToolView::GeneralPurpose;
        app.general_tool_tab = GeneralToolTab::Design;
        let id = app
            .general_design_document
            .add_circle(Point::new(0.0, 0.0), 8.0)
            .expect("valid editable circle");
        let mut harness = Harness::builder()
            .with_size(egui::vec2(1000.0, 700.0))
            .build_eframe(|_| app);
        ...
        harness.state_mut().begin_general_object_edit(id);
        harness.run();
    ...
    fn general_circle_tool_svg_is_valid() {
        ...
        let edit_svg = include_str!("../assets/tool-icons/edit-parameters.svg");
        assert!(edit_svg.contains("viewBox=\"0 0 24 24\""));
        assert!(!general_edit_parameters_icon_strokes().is_empty());
    }
