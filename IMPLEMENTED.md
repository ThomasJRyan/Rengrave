...
- Finish hardening the native Potrace port beyond the current flower smoke: tree/list ordering, byte-level DXF comparisons, threshold/input-format edge cases, and additional C Potrace/F-Engrave bitmap fixtures still need coverage.
- Bitmap parity against Potrace/F-Engrave fixtures, including image thresholding, alpha behavior, option edge cases, and fixture-proven DXF/toolpath output beyond `Flower.jpg`.
