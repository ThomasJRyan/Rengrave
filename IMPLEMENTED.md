...
The UI is now an MVP rather than only a shell, but it still lacks several expected desktop workflow pieces. Native dialogs are attempted with in-app fallback, but there are still no rich font/image preview editing controls, cooperative mid-algorithm cancellation, or full F-Engrave-style menu coverage. Controls cover common and several advanced settings, but not every legacy knob. Background jobs keep the UI responsive and now report coarse phases, but the current Cancel action ignores late worker results rather than interrupting every core loop immediately.
