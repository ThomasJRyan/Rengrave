...
- The left panel also shows a cached input preview: CXF/TTF strokes from the current engraving text sample with missing-character warnings, DXF line artwork with stroke length/extents/range readouts, or bitmap original/trace-mask thumbnails with black/white coverage, with decode/parser errors shown inline and a Refresh action for changed files.
