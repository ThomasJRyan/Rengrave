...
- A small arc CXF fixture was added under `crates/rengrave-core/tests/fixtures/inputs/` so arc fitting is represented in the crate-level integration fixture set.
...
- A crate-level golden-output harness now exists under `crates/rengrave-core/tests/golden.rs` with minimal CXF, arc CXF, and DXF fixtures, checked G-code/SVG regression outputs, numeric-tolerant G-code comparison helpers, and end-to-end Rust batch cases for multiline text, text-on-circle Add Circle output, flat-text Add Box output, arc-fit formats, transform/settings round trips, and DXF SVG/DXF export generation. These tests pin current R-Engrave output; they are not yet F-Engrave-generated parity fixtures.
...
3. Continue expanding parity tests for bitmap imports, additional Add Box edge cases, TTF conversion, cleanup, and V-carve/inlay outputs; multiline text, text-on-circle, basic Add Box, arc-fit formats, transform/settings round trips, and basic DXF import now have integration coverage.
